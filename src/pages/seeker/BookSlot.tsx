            ...
            <Card className="shadow-md">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg border border-blue-200">
                  ...
                  <Switch checked={isMultiDateMode} onCheckedChange={handleMultiDateModeToggle} />
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    {isMultiDateMode && selectedDates.length > 0 && <Badge variant="outline" className="bg-green-50 text-green-700 border-green-300">
                        {selectedDates.length} day{selectedDates.length > 1 ? 's' : ''} selected
                      </Badge>}
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {availableDates.map(date => <Button key={date} variant={selectedDates.includes(date) ? "default" : "outline"} onClick={() => handleDateSelect(date)} className={`px-4 py-2 rounded-lg transition-all ${selectedDates.includes(date) ? 'bg-[#FF6B00] hover:bg-[#FF6B00]/90 text-white' : 'border-gray-300 hover:border-[#FF6B00] hover:text-[#FF6B00]'}`}>
                ...
                {isMultiDateMode && selectedDates.length > 0 && <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                    <h4 className="font-semibold text-green-900 mb-2">Selected Dates</h4>
                    <div className="flex flex-wrap gap-2">
                      {selectedDates.map(date => <div key={date} className="flex items-center space-x-2 bg-white border border-green-200 rounded-lg px-3 py-1 shadow-sm">
                ...
                {selectedDates.length > 0 && availableSlotsForSelection.length > 0 && <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {availableSlotsForSelection.map(slot => <div key={slot.id} className="flex items-center space-x-2 bg-white border border-blue-200 rounded-lg px-3 py-2 shadow-sm">
                ...
                {selectedDates.length > 0 && availableSlotsForSelection.length > 0 && <Card className="p-4 bg-[#F8F9FA] border shadow-sm hover:shadow-md transition-shadow duration-200 rounded-xl">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 items-end">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <Select value={singleSlotBooking.slotId} onValueChange={value => updateSingleSlotBooking('slotId', value)}>
                          ...
                          <SelectContent>
                            {availableSlotsForSelection.map(slot => <SelectItem key={slot.id} value={slot.id.toString()}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <Input type="text" placeholder="e.g., TN 10 AK 6567" value={singleSlotBooking.vehicleNumber} onChange={e => updateSingleSlotBooking('vehicleNumber', e.target.value)} className="h-9 text-sm mt-1 focus:ring-2 focus:ring-[#FF6B00]/20" required />
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <Select value={singleSlotBooking.startTime} onValueChange={value => updateSingleSlotBooking('startTime', value)} disabled={!singleSlotBooking.slotId}>
                          ...
                          <SelectContent>
                            {getAvailableStartTimes(singleSlotBooking.slotId).map(time => <SelectItem key={time} value={time}>{time}</SelectItem>)}
                          </SelectContent>
                        </Select>
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <Select value={singleSlotBooking.endTime} onValueChange={value => updateSingleSlotBooking('endTime', value)} disabled={!singleSlotBooking.startTime}>
                          ...
                          <SelectContent>
                            {getAvailableEndTimes(singleSlotBooking.slotId, singleSlotBooking.startTime).map(time => <SelectItem key={time} value={time}>{time}</SelectItem>)}
                          </SelectContent>
                        </Select>
                      </div>
                ...
                {/* No slots message */}
                {selectedDates.length > 0 && availableSlotsForSelection.length === 0 && <div className="text-center py-8 text-gray-500">
            ...
            <div className="sticky top-4">
              <Card className="bg-[#FFF8F2] border-[#FF6B00]/30 shadow-lg rounded-xl">
                ...
                <CardContent className="space-y-3">
                  {selectedDates.length > 0 && <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                      <div className="text-sm font-medium text-blue-900">
                        Selected Date{selectedDates.length > 1 ? 's' : ''}
                      </div>
                      <div className="text-blue-700 space-y-1">
                        {selectedDates.map(date => <div key={date} className="text-sm">
                            {new Date(date).toLocaleDateString('en-US', {
                        weekday: 'short',
                        month: 'short',
                        day: 'numeric',
                  ...
                  {singleSlotBooking.slotId && <div className="space-y-2">
                      <div className="text-sm">
                        <span className="font-semibold text-[#1C1C1C]">Slot Configuration</span>
                        {singleSlotBooking.vehicleNumber && <div className="text-xs text-[#666]">
                            Vehicle: {singleSlotBooking.vehicleNumber}
                          </div>}
                        {singleSlotBooking.startTime && singleSlotBooking.endTime && <div className="text-xs text-[#666]">
